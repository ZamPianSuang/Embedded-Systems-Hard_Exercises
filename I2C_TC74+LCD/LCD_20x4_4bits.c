...
void GPIO_Init(void)
{
	...
	GPIOB->MODER &= ~(GPIO_MODER_MODE5 | GPIO_MODER_MODE8);
	GPIOB->MODER |=  (GPIO_MODER_MODE5_0 | GPIO_MODER_MODE8_0);			// PB5 & PB8 output mode
	GPIOC->MODER &= ~0xFF00U;
	GPIOC->MODER |=  0x5500U;			// PC4 - PC7 output mode
	
	GPIOB->PUPDR &= ~(GPIO_PUPDR_PUPD5 | GPIO_PUPDR_PUPD8);			// PB5 & PB8 No pull-up, pull-down
